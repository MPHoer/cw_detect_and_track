"""
Generate forward tracking variants for visual comparison; no GT ranking.

GT-free version of forward_trackpy/pp_fill_holes_optim.py. See this folder's README for differences.
...
RESULTS_DIR = Path(r"D:\cw_rf_detr_dataset_raw\tracking_inference\trackpy_parameter_sweep")
RESOLUTION = 512
...
HYPERPARAM_SEARCH_SPACE = {'TRACKPY_SEARCH_RANGE': [20, 30, 40], 'TRACKPY_MEMORY': [10, 20], 'MAX_HOLE_LEN': [5, 10, 20], 'HOLE_MIN_EDGE_CONF': [0.5, 0.6, 0.7]}
...
def main():
    ...
    settings = {k: str(v) if isinstance(v, Path) else v for k, v in globals().items()
                if k.isupper()}
    keys = list(HYPERPARAM_SEARCH_SPACE)
    for index, values in enumerate(itertools.product(*(HYPERPARAM_SEARCH_SPACE[k] for k in keys))):
        cfg = dict(zip(keys, values))
        processed = track(copy.deepcopy(store), sequences, classes, cfg)
        export_results(processed, sequences, images, names, IMAGE_ROOT,
                       RESULTS_DIR / f'exp_{index:03d}', {**settings, **cfg})
    print('Sweep complete. No best configuration selected: ground truth is not available.')
