...
Run only the helpers relevant to your data. The 20×20 helper's output name differs from the historical trainer's `_perfect_gt_annotations_20x20.coco.json`; select or rename deliberately.
...
Open [src/model/train_rfdetr.py](src/model/train_rfdetr.py). There are two ways to select data:

**Your own prepared COCO dataset, with or without ASM:** inside the main block, replace the existing setup from `SELECT_ASM = True` through the `select_annotations(...)` call with:

```python
dataset_path = r"D:\my_dataset"  # your actual dataset root
run_name = "RGB_cells"          # use a descriptive, unique name
SELECT_GT_VARIANT = "prepared"  # label for the existing status print only
```

Keep the subsequent output-directory creation and training code. Do not call `select_annotations()` in this route: your splits already contain the desired `_annotations.coco.json`. For prepared ASM data the same setup works, with an appropriate run name. **Setting `SELECT_ASM=False` alone does not work** in the original script because paths would be undefined. The README describes this edit; it has not changed the script for you.

**Reproduce the historical annotation selection:** retain the original setup, set `dataset_path`, keep `SELECT_ASM=True`, and choose `SELECT_GT_VARIANT="perf_gt20"` for the historical `ASM_perf_gt20` run. Other supported values are `perf_gt` and `old_gt`; see the actual `name_map` in the function (its older Alex/hoechst comment is stale). Selection overwrites each split's active annotation file without backup. Missing variants only warn and may leave old labels active.
