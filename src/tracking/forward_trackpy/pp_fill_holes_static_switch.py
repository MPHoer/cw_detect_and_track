"""
Run forward Trackpy tracking, gap filling, and nearby-ID continuation cleanup.
...
and no score bonus. Edit the outer TRACK_MERGE_DIST_PX list to test merge
radii; the entire pipeline reruns for each value. After interpolation,
merge an existing ID into a continuation when the nearest detection in the
next frame starts a new ID within this radius. This is ID cleanup, not a
switch between static and velocity-based motion models. Radius 0 still
allows exact-position matches. No short-segment pruning is performed.
...
for TRACK_MERGE_DIST_PX in [0]:
    ...
    RESULTS_DIR = os.path.join(BASE_DIR, "results_rfdetr_eval", "trackpy_fill_holes_velocity_static_switch", CHECKPOINT_NAME)
    ...
    # track ID switch cleanup hyperparam
    # TRACK_MERGE_DIST_PX = int(4.0)    # px: max center distance to consider "same cell" for ID merge
    ...
    def merge_track_ids_near_static(
        all_tracks_by_seq_cls,
        seq_to_img_ids,
        merge_dist_px=TRACK_MERGE_DIST_PX,
    ):
        """
        ...
        - If that neighbour at frame f+1 belongs to a "new" track B whose first
            appearance is exactly at f+1, treat B as a continuation of A and
            merge B into A (rename all B's detections to track_id A).
        """
        merge_count = 0
        ...
        for seq_name, cls_tracks in all_tracks_by_seq_cls.items():
            ...
            # frame index mapping within this sequence
            frame_idx_to_img_id = {fi: img_id for fi, img_id in enumerate(seq_img_ids)}
            img_id_to_frame_idx = {img_id: fi for fi, img_id in frame_idx_to_img_id.items()}
            ...
            for cls, tracks in cls_tracks.items():
                ...
                # we might update tracks list after merges; track ids can change
                # so we run a small fixed-point loop until no more merges happen.
                changed = True
                ...
                while changed:
                    ...
                    # refresh frame_to_dets & track_frames (because we might have changed IDs)
                    track_frames.clear()
                    frame_to_dets.clear()
                    for trk in tracks:
                    ...
                    for fi in range(num_frames - 1):
                        ...
                        for det in dets_f:
                            ...
                            # require that this track already existed BEFORE this frame
                            # (so it's a "stable" old track)
                            frames_pid = track_frames.get(pid, set())
                            if not frames_pid or min(frames_pid) >= fi:
                                # no earlier presence -> skip; we only merge new IDs into older ones
                                continue

                            # find nearest neighbour in next frame with a different track_id
                            best_det2 = None
                            best_dist = float("inf")
                            ...
                            # require that the "new" track starts exactly at fi+1
                            ...
                            # reassign all detections from qid to pid
                            for fi2, img2, det2 in trk_qid["detections"]:
                                det2["track_id"] = pid
                                trk_pid["detections"].append((fi2, img2, det2))

                            # mark qid's track as removed
                            removed_track_ids.add(qid)
                            merge_count += 1
                            changed = True
                            # after a merge, restart scanning (outer while) to use updated structures
                            break
                        if changed:
                            break  # break fi loop
                    # end for fi
                ...
                # actually remove merged-away tracks from list
                if removed_track_ids:
                    new_tracks = []
                    for trk in tracks:
                        if trk["id"] in removed_track_ids:
                            continue
                        # ensure detections are sorted and last_frame/last_box updated
                        if trk["detections"]:
    ...
    print(f"[DEBUG] Track ID switch cleanup DONE: merged_track_pairs={merged_pairs}")
    sys.stdout.flush()
    ...
    for THR in CONF_THR_LIST:
        video_path = os.path.join(
            RESULTS_DIR,
            f"video_MODEL_{CHECKPOINT_NAME}__GT{GT_ANNOTATION}__thr{THR:.2f}__{TRACK_MERGE_DIST_PX}.mp4",
        )
