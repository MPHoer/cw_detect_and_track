"""
Evaluate an RF-DETR Medium checkpoint against COCO bounding-box annotations.

Load prepared validation images as RGB and run detection with threshold=0.0.
Evaluate classes present in non-crowd annotations, excluding 'dummy'.

Report per-class precision, recall, and F1 at each confidence cutoff
(default 0.4), using two independent matching criteria:
- Bounding-box IoU >= 0.5.
- Box-centre distance <= 10 pixels.

Matching is greedy, highest-confidence first, and one-to-one within each
image and class. Counts are pooled across images before computing scores.

Compute AP from all returned detection scores using a VOC-style precision
envelope, at IoU=0.5 and across IoU=0.50–0.95. Average AP across evaluated
classes to obtain mAP. This is a custom calculation, not official COCO AP.

The current configuration compares ASM_old_gt/checkpoint_best_ema.pth
with valid/_perfect_gt_annotations_20x20.coco.json. Missing images and
failed inference calls retain their ground truth as missed detections.

Write text and JSON metrics to RESULTS_DIR, overwriting matching filenames.
No training, tracking, image preprocessing, or prediction export is performed.

Configure paths and thresholds before running. Execution also occurs on import.
"""
...
for img_id, im_meta in imgid_to_info.items():
    try:
        ...
        preds = model.predict(image, threshold=0.0)
...
summary_lines = []
summary_lines.append("==== RF-DETR EVAL SUMMARY ====\n")
