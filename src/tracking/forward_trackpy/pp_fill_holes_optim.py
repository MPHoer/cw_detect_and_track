"""
Search forward Trackpy and gap-filling parameters against annotated tracks.
...
Edit HYPERPARAM_SEARCH_SPACE to set the candidate search ranges, memories,
hole lengths, and endpoint confidences; these override the same-named
single-run defaults. The current grid contains 54 combinations.
Run inference once, then copy the cached predictions for each combination.
Select the highest custom global IDF1 at CONF_THR_LIST[0], saving per-run
reports and hp_search_results.txt. Rerun the winning tracking configuration
with video output, reusing detections. No static-ID merging or pruning is
included. Ground-truth track IDs are needed for meaningful selection;
reserve separate sequences for evaluation after tuning.
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_rfdetr_eval", "trackpy_fill_holes_velocity_hpsearch")
...
TRACK_MAX_AGE = 30        # used as a "memory"-like scale for trackpy (for logging)
...
# trackpy-specific hyperparams (defaults; will be overridden in search)
TRACKPY_SEARCH_RANGE = 40     # pixels: max displacement between frames
TRACKPY_MEMORY = 20           # frames: how many frames a particle can vanish

# hole filling hyperparams (defaults; will be overridden in search)
MAX_HOLE_LEN = 20          # maximum number of consecutive missing frames to fill
...
HYPERPARAM_SEARCH_SPACE = {
    "TRACKPY_SEARCH_RANGE": [20, 30, 40],
    "TRACKPY_MEMORY": [10, 20],
    "MAX_HOLE_LEN": [5, 10, 20],
    "HOLE_MIN_EDGE_CONF": [0.5, 0.6, 0.7],
}
...
# Keep a pristine copy of predictions to reuse across hyperparameter configs
raw_pred_store = copy.deepcopy(pred_store)

# =====================================================
# --- tracking metrics (CLEAR MOT + global IDF1) ------
# =====================================================

def compute_clear_mot_metrics(pred_store_local, gt_tracks_by_img_cls_local, THR):
    """
    ...
    for seq_name, seq_img_ids in seq_to_img_ids.items():
        ...
        for cls in CLASSES:
            ...
            for img_id in seq_img_ids:
                gt_objs = [
                    g for g in gt_tracks_by_img_cls_local[img_id][cls]
                    if g["track_id"] is not None
                ]
                pred_objs = [
                    d for d in pred_store_local[img_id][cls]
...
def compute_global_id_metrics(pred_store_local, gt_tracks_by_img_cls_local, THR):
    """
    ...
    total_gt_tracks_local = len(set(k[2] for k in gt_track_keys))  # approximate
...
def compute_pred_track_length_stats(all_tracks_by_seq_cls_local, THR):
    """
    ...
    for seq_name, cls_tracks in all_tracks_by_seq_cls_local.items():
...
# =====================================================
# --- helper: run one hyperparameter config -----------
# =====================================================

def run_tracking_and_eval_one_config(
    cfg,
    raw_pred_store,
    make_video=False,
    exp_name="exp"
    ...
    """
    cfg: dict with TRACKPY_SEARCH_RANGE, TRACKPY_MEMORY, MAX_HOLE_LEN, HOLE_MIN_EDGE_CONF
    raw_pred_store: pristine detections (no tracks, no hole filling)
    make_video: if True, also create video for this config
    exp_name: string used for subdirectory under RESULTS_DIR
    """
    TRACKPY_SEARCH_RANGE = cfg["TRACKPY_SEARCH_RANGE"]
    TRACKPY_MEMORY = cfg["TRACKPY_MEMORY"]
    MAX_HOLE_LEN = cfg["MAX_HOLE_LEN"]
    HOLE_MIN_EDGE_CONF = cfg["HOLE_MIN_EDGE_CONF"]

    print(f"\n[HP-SEARCH] Running config {exp_name}: "
          f"search_range={TRACKPY_SEARCH_RANGE}, memory={TRACKPY_MEMORY}, "
          f"MAX_HOLE_LEN={MAX_HOLE_LEN}, HOLE_MIN_EDGE_CONF={HOLE_MIN_EDGE_CONF}")

    # deep copy raw predictions so we can freely modify
    pred_store_local = copy.deepcopy(raw_pred_store)

    # all_tracks_by_seq_cls[seq][cls] -> list of track dicts
    all_tracks_by_seq_cls_local = defaultdict(lambda: defaultdict(list))

    # --- tracking with trackpy (velocity predictor) ---
    for seq_name, seq_img_ids in seq_to_img_ids.items():
        if not seq_img_ids:
            continue
        ...
        for cls in CLASSES:
            ...
            for img_id in seq_img_ids:
                ...
                dets = pred_store_local[img_id][cls]
            ...
            if not rows:
                all_tracks_by_seq_cls_local[seq_name][cls] = []
                continue
            ...
            for _, row in linked.iterrows():
                ...
                det = pred_store_local[img_id][cls][det_idx]
                det["track_id"] = pid
            ...
            all_tracks_by_seq_cls_local[seq_name][cls] = tracks

    print("[DEBUG] Trackpy tracking DONE for config:", exp_name)
    sys.stdout.flush()
            ...
            for trk in tracks:
                ...
                for idx_d in range(n - 1):
                    ...
                    for k in range(1, gap + 1):
                        ...
                        pred_store_local[mid_img_id][cls].append(new_det)
                        trk["detections"].append((mid_frame_idx, mid_img_id, new_det))
                        interpolated_count += 1

    print(f"[DEBUG] Hole filling DONE (config {exp_name}): interpolated_detections={interpolated_count}")
    sys.stdout.flush()

    # --- count detections after postproc ---
    postproc_preds_total = 0
    for img_id, per_cls in pred_store_local.items():
        for cls in CLASSES:
            postproc_preds_total += len(per_cls[cls])

    print(
        f"[DEBUG] Tracking + score boosting + hole filling DONE (config {exp_name}): "
    ...
    for THR in CONF_THR_LIST:
        ...
        for img_id, im_meta in imgid_to_info.items():
            for cls in CLASSES:
                ...
                dets_scored = pred_store_local.get(img_id, {}).get(cls, [])
    ...
    print(
        f"[DEBUG] Detection evaluation DONE (config {exp_name}): "
        f"raw_preds_total={raw_preds_total}, postproc_preds_total={postproc_preds_total}"
    )
    sys.stdout.flush()

    # --- metrics + logging for this config ---
    summary_lines = []
    summary_lines.append("==== RF-DETR EVAL + TRACKING (trackpy + hole-filled, per-sequence) ====\n")
    ...
    for THR in CONF_THR_LIST:
        ...
        clear_metrics = compute_clear_mot_metrics(pred_store_local, gt_tracks_by_img_cls, THR)
        ...
        id_metrics = compute_global_id_metrics(pred_store_local, gt_tracks_by_img_cls, THR)
        id_line = (
        ...
        track_len_stats = compute_pred_track_length_stats(all_tracks_by_seq_cls_local, THR)
    ...
    ap_per_class = {}
    for cls in CLASSES:
        ap, recs, pres = compute_ap_for_class(pred_store_local, gt_by_img_cls, cls, IOU_THR)
        ap_per_class[cls] = ap
    ...
    for iou in IOU_LIST:
        ap_per_class_iou = {}
        for cls in CLASSES:
            ap_iou, _, _ = compute_ap_for_class(pred_store_local, gt_by_img_cls, cls, iou)
    ...
    # --- write logs for this config ---
    exp_dir = os.path.join(RESULTS_DIR, "hp_search", exp_name)
    ...
    print(f"\n[INFO] Config {exp_name} results written to:")
    print(f"  {summary_path}")
    print(f"  {metrics_path}")

    # --- optionally create video for this config (for inspection) ---
    if make_video:
        print(f"[INFO] Creating video for best config in {exp_name}")
        ...
        for THR in CONF_THR_LIST:
            ...
            for idx, img_id in enumerate(all_img_ids_sorted):
                ...
                per_cls_preds = pred_store_local.get(img_id, {})
                for cls in CLASSES:
    ...
    # objective: IDF1 at the first (and only) threshold
    THR0 = CONF_THR_LIST[0]
    id_metrics0 = metrics_dict["per_threshold"][str(THR0)]["tracking_IDF1_global"]
    idf1 = id_metrics0["IDF1"]
...
hp_results_path = os.path.join(RESULTS_DIR, "hp_search_results.txt")
hp_lines = []
hp_lines.append("Hyperparameter search over trackpy + hole-filling\n")
hp_lines.append(str(HYPERPARAM_SEARCH_SPACE) + "\n")
...
values_product = list(itertools.product(*[HYPERPARAM_SEARCH_SPACE[k] for k in keys]))

for run_idx, vals in enumerate(values_product):
    cfg = dict(zip(keys, vals))
    exp_name = f"exp_{run_idx:03d}"
    res = run_tracking_and_eval_one_config(
        cfg,
        raw_pred_store,
        make_video=False,       # videos only for best at the end
        exp_name=exp_name,
    )
    idf1 = res["idf1"]

    cfg_str = ", ".join(f"{k}={cfg[k]}" for k in keys)
    line = f"{exp_name}: IDF1={idf1:.4f}, {cfg_str}"
    print("[HP-RESULT]", line)
...
print("\n================ HP SEARCH DONE ================")
print(f"Best config: {best_exp_name} with IDF1={best_idf1:.4f}")
print("Best hyperparameters:")
for k in keys:
    print(f"  {k}: {best_cfg[k]}")
print(f"Results summary file: {hp_results_path}")

# Optionally re-run best config with video generation
print("\n[INFO] Re-running best config with video generation.")
_ = run_tracking_and_eval_one_config(
    best_cfg,
    raw_pred_store,
    make_video=True,
    exp_name=best_exp_name + "_best_with_video",
)
