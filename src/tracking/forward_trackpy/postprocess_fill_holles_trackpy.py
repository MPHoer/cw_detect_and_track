"""
Run the basic forward Trackpy detector-and-tracker with gap filling.
...
Defaults: search range 40, memory 0, hole limit 20, endpoint confidence 0.6,
and no score bonus. Memory 0 restricts reconnecting particles that disappear;
a larger hole limit alone cannot join tracks that linking has already split.
There is no later ID merging or short-segment removal.
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_rfdetr_eval", "trackpy_fill_holes_velocity")
...
TRACKPY_MEMORY = 0           # frames: how many frames a particle can vanish
...
#     for cls in CLASSES:
#         rows = []
#         # Build DataFrame rows for trackpy
...
#         # --- robust linking with adaptive shrink of search_range ---
#         try:
#             linked = tp.link_df(
#                 df,
#                 search_range=TRACKPY_SEARCH_RANGE,
#                 memory=TRACKPY_MEMORY,
#                 adaptive_stop=1,      # minimum allowed search_range
#                 adaptive_step=0.95,   # multiply search_range by this until subnets are small enough
#             )
#         except SubnetOversizeException as e:
#             # fallback: try again with a smaller fixed search_range
...
#         # Drop any rows without a particle assignment
#         linked = linked.dropna(subset=["particle"])
#         if linked.empty:
#             all_tracks_by_seq_cls[seq_name][cls] = []
#             continue

#         # Assign track_ids back to detections and group by particle
